...
public class LexiconWrapper {
     ...
     * 
     * @param configFile	The file from which to read Lexicon configuration.
     * 
     * @return the singleton Lexicon access object
     */
    public static LexiconWrapper getInstance(String configFile) {
		if (lexWrapper == null) {
		    lexWrapper = new LexiconWrapper(configFile);
		}
		return lexWrapper;
    }
    ...
    /**
     * Queries the Lexicon for lexical entries for a given list of tokens. The lexical entries are longest-matching items.
     * 
     * @param tokens		the input list of tokens
     * @return	a list of <code>LexiconMatch</code> objects that represent the lexical entries corresponding to the list of input tokens
     * 
     * @throws SQLException when there is a problem with accessing the Lexicon
     */
...
/*    		if (StringUtils.isAllUpperCase(w.getText()) == false && (w.getPos().equals("IN") || w.getPos().equals("TO"))) {
	    		for (LexRecord r: inrecs) {
	    			if (r.GetCategory().equals("prep") == false) continue;
	    			out.add(r);
	    		}
    		}  else return in;*/
    		...
    		if (s.toLowerCase().equals("lower")) {
    			for (LexRecord r: inrecs) {
    				if (r.GetBase().equals("lour")) continue;
    				out.add(r);
    			}
    		}  else {
	    		return in;
    ...
    /**
     * Given a list of lexical matches, filter out those where the POS tag is not compatible with the  tag found through 
     * POS tagging. 
     * 
     * @param lexmatches		The list of lexical matches to check
		...
		for (LexiconMatch match: lexmatches) {
			...
			if (tokens.size() == 1) {
	    		...
	    		for (LexRecord r: lexrecs) {
	    			if (possiblePos!= null && possiblePos.contains(r.GetCategory()) == false) continue;
	    			upd.add(r);
	    		}
